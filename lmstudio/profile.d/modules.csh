# Module
source /usr/local/lmod/lmod/init/cshrc
set GROUPMODPATH=`echo ${HOME} | sed -e 's/\/'${USER}'$/\/shared\/modulefiles/g'`
if ( -d $GROUPMODPATH ) then
        set mp=`/usr/local/lmod/lmod/libexec/addto  --append MODULEPATH $GROUPMODPATH`
        setenv MODULEPATH $mp
endif
set mp=`/usr/local/lmod/lmod/libexec/addto  --append MODULEPATH /util/opt/hcc-modules/Common`
setenv MODULEPATH $mp
...
if ( ! $?__Init_Default_Modules )  then
  ...
  if ( ! $?LMOD_SYSTEM_DEFAULT_MODULES ) then
    setenv LMOD_SYSTEM_DEFAULT_MODULES "$SLURM_CLUSTER_NAME"
  endif
